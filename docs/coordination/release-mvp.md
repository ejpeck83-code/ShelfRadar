...
DNS rebinding and edge rate limiting remain reviewed connector/hosting controls. Basic owner auth has no MFA/session UI. No production source is live. Restore verification, current PostgreSQL CI, Playwright/axe, clean-clone verification, fixture preview deployment, and preview smoke remain release gates. Do not merge or tag until those gates are green and the user explicitly authorizes release.
